...
df_jp_ind = pd.read_csv('./csv_data/一人当たり賃金_全国_全産業.csv', encoding='shift_jis')
df_jp_category = pd.read_csv('./csv_data/一人当たり賃金_全国_大分類.csv', encoding='shift_jis')
df_pref_ind = pd.read_csv('./csv_data/一人当たり賃金_都道府県_全産業.csv', encoding='shift_jis')

jp_lat_lon = pd.read_csv('./pref_lat_lon.csv')
jp_lat_lon = jp_lat_lon.rename(columns={'pref_name': '都道府県名'})


st.subheader('■2019年：一人当たり平均賃金のヒートマップ', )
...
view = pdk.ViewState(longitude=139.691648, latitude=35.689185, zoom=6, pitch=40.5)
layer = pdk.Layer('HeatmapLayer', data=df_pref_map, opacity=0.4, get_position=['lon', 'lat'], threshold=0.3, get_weight='一人当たり賃金（相対値）')
layer_map = pdk.Deck(layers=layer, initial_view_state=view)

st.pydeck_chart(layer_map)

# show_df = st.checkbox('データ表示')
# if show_df == True:
#     st.write(df_pref_map)


st.subheader('■集計年別の一人当たり賃金（万円）の推移')
...
st.subheader('■年齢階級別の全国一人当たり平均賃金（万円）')
